...
def test_cli_reclassify_dispatches(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("slopmortem.cli._app.reclassify_quarantined", fake_reclassify)
    monkeypatch.setattr("slopmortem.cli._app._build_journal", _fake_journal)
    monkeypatch.setattr("slopmortem.cli._app._build_slop_classifier", _fake_classifier)
