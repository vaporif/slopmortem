...
Five parallel reviewers swept the design spec across pipeline data-flow, external API usage, internal consistency, concurrency / failure handling, and schema coherence. A sixth agent re-verified each finding independently with file:line evidence and external doc lookups (lmnr-python source, OpenRouter docs). This file is the deduped, verified punch-list.

All edits land in `docs/specs/2026-04-27-slopmortem-design.md`. No code lands from this plan; the underlying behaviors are Task #N implementer responsibilities, but the spec contracts they read have to be correct first.
...
**Fix:** Specify `return_exceptions=True` on the gather (or wrap each per-candidate coroutine in a try/except that returns a sentinel `SynthesisFailure` value), and update the prose at 678–684 to name the chosen mechanism. The reporting path at 719 then filters for the sentinel/exception type.
...
These don't crash on first call, but they force the implementer to invent a contract and ship something that diverges from the spec.
...
**Problem:**
- `ToolSpec` is listed as belonging to both `slopmortem/llm/tools.py` (316) and `slopmortem/models.py` (410). Two import paths for the same name means either a circular import or the implementer guesses wrong.
...
- **Cite line for facet plural-vs-singular:** the original review pointed at line 273 as evidence of singular `sector`; line 273 doesn't actually contain that text. The underlying ambiguity (Blocker 7 above) is real, but specific line citations have to come from wherever `Facets` is declared once that exists.
...
- **`@observe(ignore_inputs)` on top-level params:** filters correctly when the input is a top-level argument; the failure mode is specifically dotted paths (Blocker 3).
