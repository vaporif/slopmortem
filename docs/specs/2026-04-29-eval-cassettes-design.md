...
## Plan review findings (2026-04-29, second pass against `docs/plans/2026-04-29-eval-cassettes.md`)

5 reviewers (correctness, type-safety, test-strategy, architecture, determinism) + 2 validators against the implementation plan. Findings below are validator-confirmed against the **plan's actual code blocks** (not the spec text). They surface bugs the spec-level review (B/G/M/F above) could not see because the plan-level code didn't exist yet. Severity: **P-BLOCKER** = plan won't compile, ship, or replay correctly as written; **P-CRITICAL** = quietly weakens a spec-promised invariant; **P-MAJOR** = friction/style mismatch that won't break CI but diverges from project norms.

**Auto-fixed in plan (2026-04-29 commit on this branch):** P2 (Task 1 Modify list now includes the 5 missed files + `runner.py:_build_canned`), P3 (record path now imports `_row_id` from runner), P5/P9 partial (`run_ingest` → `ingest` function-name fix; cost-ceiling threading and pass-criteria still open), P8 (Task 1D migration table now names `test_synthesize_all_warms_cache_before_gather`, `test_ingest_bounded_fan_out_concurrency`, `test_ingest_cache_read_ratio_warning`), P10 (all `@pytest.mark.anyio` decorators dropped, `anyio_backend` fixture instructions removed, `import anyio`/`anyio.run` → `import asyncio`/`asyncio.run`). Remaining open items below.

### P-BLOCKER (2)

- **P1 — `template_sha` helper is dead code; runtime cassette key still uses `.j2`-only hash.** Plan `cassettes.py` (plan lines 238-258) defines `template_sha(template_text, tools, response_format)` per spec B2's `[autofix]`, but **no code calls it on the recording path**. Stages still pass `prompt_template_sha(name)` from `slopmortem/llm/prompts/__init__.py:46-48` (`hashlib.sha256(<file_bytes>).hexdigest()[:16]`); `RecordingLLMClient` (plan lines 1168-1170) accepts that 16-hex value verbatim into the cassette key. Editing `synthesis_tools()` or the `Synthesis` Pydantic schema produces a new live response with the same cassette key — silent stale replay. The exact failure mode B2 was meant to fix is reintroduced at the plan level. `[autofix]` make `prompt_template_sha` accept and fold tools+response_format and have stages pass them, OR extend the cassette key to a 4-tuple including `tools_schema_sha` and have `RecordingLLMClient` compute it from the call's `tools`/`response_format` args (preferred — keeps stages decoupled from cassette layout).

- **P4 — `RecordingLLMClient` cost ceiling has off-by-one + soft-cap design bug; the unit test as written cannot pass.** Plan line 1154 pre-call check: `if self._max_cost_usd is not None and self._spent_usd >= self._max_cost_usd:` — i.e. only fires *after* spending equals or exceeds the cap. Plan line 1193 settles `self._spent_usd += result.cost_usd` *post-call*. The Task 1F unit test (plan lines 1019-1034) configures three calls of `cost_usd=0.40` with `max_cost_usd=1.00` and asserts `RecordingBudgetExceededError` on the 3rd call with `exc_info.value.spent == pytest.approx(0.80)`. Walked through: call 1 (spent 0→0.40), call 2 (0.40→0.80), call 3 (check 0.80 ≥ 1.00 is False, proceed, spent 0.80→1.20) — no raise. Test fails as written. Even fixing the test, the design is a soft cap: a single $5 call when `spent=$1.99` blows past a `$2.00` ceiling. The spec's F20 resolution (default `$2.00 = 2× estimate`) loses its bite. `[discuss + autofix]` (a) flip the check to post-call accumulate-then-test (`if spent + cost_usd > limit: raise after settling`), or (b) make the ceiling hard with a per-call worst-case reservation. Either way, recompute the test scenario so the cap is actually crossed by the call the test asserts on — e.g. set `cost_usd=0.40` and `max_cost_usd=0.79` so the 3rd call sees `spent=0.80 >= 0.79`. Document soft-vs-hard semantics on `--max-cost-usd`.

### P-CRITICAL (4)

- **P5 — `eval-record-corpus` ingest path is uncapped; spec's cost-ceiling guarantee silently doesn't apply to corpus regen.** Spec line 52 promises `RecordingLLMClient` is the guardrail against "runaway tool loops or model-upgrade surprises." Plan Task 3 (`record_cassettes_for_inputs`, plan lines 1860-1930) wraps only `pipeline.run_query`. Plan Task 5's `corpus_recorder.py` calls into `slopmortem.ingest.ingest` (function-name fix landed) but the `LLMClient` it threads in is the live one. None of the ingest stages — `_facet_summarize_fanout`, `_cache_warm`, dense+sparse embedding — are wrapped by `RecordingLLMClient`. Operator running `eval-record-corpus` has no programmatic cost cap; one model-upgrade surprise can blow $5+ silently. `[autofix]` thread `RecordingLLMClient(max_cost_usd=...)` through `corpus_recorder.py`'s call and document the cap default in §"Recording flow" alongside `eval-record`'s.

- **P6 — Git LFS attribute set in Task 4, but no precommit gate; non-LFS commit silently possible.** `/Users/vaporif/Repos/premortem/.gitattributes` does not exist today; `.pre-commit-config.yaml` does not exist. Plan Task 4 (line 1973) adds `tests/fixtures/corpus_fixture.jsonl filter=lfs diff=lfs merge=lfs -text`. Task 5 operator step (lines 2173-2178) lists `git lfs install` once but jumps straight to `just eval-record-corpus`. The validation step (line 2249, `git lfs ls-files shows ... tracked by LFS`) runs **post-commit**; an operator who ran `git lfs install` only globally on a stale checkout (or in a fresh worktree without the per-checkout hook) ships a 1.5 MB raw blob. Once pushed, history rewrite is the only fix. `[autofix]` add an explicit pre-commit check in Task 5: `git check-attr filter -- tests/fixtures/corpus_fixture.jsonl` must print `filter: lfs` before `git add`; otherwise abort and run `git lfs install` in the checkout.

- **P7 — Task 7 silently weakens `test_full_pipeline_with_fake_clients` from a 14-assertion contract test to a 1-assertion smoke test.** Original (`tests/test_pipeline_e2e.py:259-290`) asserts: `report.input == ctx`, `0 < len(report.candidates) <= cfg.N_synthesize`, `meta.K_retrieve == cfg.K_retrieve`, `meta.cost_usd_total == budget.spent_usd`, `meta.budget_exceeded is False`, `meta.trace_id is None`, `set(meta.models.keys()) == {"facet","rerank","synthesize"}`, `meta.models["facet/rerank/synthesize"]` exact strings, `"facet_extract" in progress_events`, `len(fake_corpus.queries) == 1`, plus `q["k_retrieve"]` and `q["strict_deaths"]` round-trip. Plan migration (lines 2589-2590): `assert len(report.candidates) > 0  # ... preserve any existing per-candidate assertions verbatim`. The comment is not executable; an executor following the migration drops every contract assertion. Cassette mode also drops the `_FakeCorpus` so the `fake_corpus.queries` assertions cannot port without a real `QdrantCorpus` query-instrumentation hook. `[autofix]` enumerate explicitly which original assertions must port, which need a cassette-side equivalent (e.g., observe queries via Qdrant span instrumentation or add `_record_query` callback to the helper), and which are demonstrably incompatible with cassettes; remove the placeholder comment.

- **P9 — Task 5 has no objective pass/fail criteria.** Function-name half landed (`run_ingest` → `ingest`). Open: validation (lines 2248-2251) still reduces to `git lfs ls-files`, "cassette files appear as regular git diffs", "operator spot-checks ~5 random `request_debug.prompt_preview` fields." Tasks 6 and 7 explicitly depend on Task 5 fixtures, but with no objective gate, "complete" is unfalsifiable and downstream tasks will land against malformed fixtures. The actual `ingest()` signature requires 10+ args (`sources, enrichers, journal, corpus, llm, embed_client, budget, slop_classifier, config, post_mortems_root`), none named `seed_inputs`. `[autofix]` (a) replace the `seed_inputs=seed_inputs, ...` placeholder with the real `ingest(...)` argument mapping (or build a dedicated seeding helper); (b) define hard pass criteria: expected cassette count = N_inputs × M_stages, every cassette validates against `load_llm_cassettes` / `load_embedding_cassettes` schema, `corpus_fixture.jsonl` line count == `len(seed_inputs)`, `just eval` against the freshly recorded set returns exit 0.

### Other plan-level findings (un-validated by the second pass; lower confidence — flag during implementation, do not treat as blockers without re-verification)

- **P11 (determinism)** — Embedding cassette key is `(model, text_hash[:16])`; loader does not assert `len(vector) == EMBED_DIMS[model]` on read. A versionless model rename or fastembed normalization tweak that changes vector length surfaces only as a downstream Qdrant write-side rejection. Add a read-side dim assertion in `load_embedding_cassettes`.
- **P12 (determinism)** — Schema version mismatch is hard-fail with no documented forward-compat policy. Adding a benign field later (e.g., `logprobs`) either bumps the version (invalidating every committed cassette → real $) or breaks readers. Document "minor schema bumps add fields, readers ignore unknowns" or check `version <= CASSETTE_SCHEMA_VERSION`. **[resolved]** Plan now uses `"<major>.<minor>"` versioning (`CASSETTE_SCHEMA_VERSION = "1.0"`); reader hard-fails on major mismatch only, accepts any minor at the same major. All envelope Pydantic models set `extra="ignore"` so future additive fields parse silently. Plan Task 1A Step 3b adds the policy + `_check_schema_version` helper; Task 1B Step 5 adds tests for major-mismatch fail and minor-bump tolerance.
- **P13 (determinism)** — `fastembed`, `onnxruntime`, `numpy` are not pinned exactly. BM25 sparse output and dense vector L2 normalization rounding can shift across versions, breaking exact-equality replay. Pin in `pyproject.toml` and record the resolved versions in `recording_metadata`; warn at replay if active versions differ.
- **P14 (types)** — `RecordingEmbeddingClient.model` (plan line 1206) reads `self._inner.model` with `# pyright: ignore[reportAttributeAccessIssue]`; the `EmbeddingClient` Protocol at `slopmortem/llm/embedding_client.py:19-24` doesn't declare `.model`. With `reportUnnecessaryTypeIgnoreComment = "error"` this is a no-win. Either add `model: str` to the Protocol or take `model` as an explicit constructor arg.
- **P15 (types)** — `_ByModelLLM.complete` (plan line 1930) declares parameters with no type annotations under non-test code; strict pyright fails on `reportMissingParameterType`. Mirror `LLMClient.complete`'s exact signature.
- **P16 (types)** — `load_llm_cassettes` uses `# pyright: ignore[reportArgumentType]` to coerce `dict[str, object]` values into typed dataclass fields (plan lines 559-566). A cassette JSON with `"cache_read_tokens": "abc"` constructs an invalid dataclass and only blows up at use site. Validate via `pydantic.TypeAdapter[LlmCassette]` instead. **[resolved]** Plan Task 1B Step 7 replaces the dataclasses with Pydantic `BaseModel` envelopes (`_LlmEnvelope`, `_EmbedEnvelope`) plus a flat `LlmCassette` consumer model. `load_llm_cassettes` / `load_embedding_cassettes` validate via `_LLM_ADAPTER.validate_python(raw)` / `_EMBED_ADAPTER.validate_python(raw)`; `ValidationError` is reraised as `CassetteFormatError(path, ...)` so a bad cassette identifies itself in the message. All `# pyright: ignore[reportArgumentType]` markers are removed.
- **P17 (architecture)** — Lazy import `from slopmortem.evals.cassettes import llm_cassette_key` inside `FakeLLMClient.complete()` papers over a layering violation: `llm/fake.py` now structurally depends on `evals/cassettes.py`. Spec G14 already flagged the inverse direction (`recording.py` location); the resolution should be moving cassette key derivation to `slopmortem/llm/cassettes.py` since key derivation is a property of the LLM contract. **[resolved]** Plan now creates `slopmortem/llm/cassettes.py` for pure key derivation (`template_sha`, `llm_cassette_key`, `embed_cassette_key`) — sibling of `slopmortem/llm/fake.py`, no `evals` dependency. `slopmortem/evals/cassettes.py` keeps the on-disk format (envelope models, loaders/writers, slugifier, schema-version policy, error types). `FakeLLMClient`, `RecordingLLMClient`, the conftest helper, and the runner all import key derivation from `slopmortem.llm.cassettes` at top level — no lazy imports, no cycle. Plan Task 1A Step 3a/3b implements the split.
- **P18 (architecture)** — Task 6 collapses `allowed_hosts` from `payload.sources`-augmented to the fixed allowlist (plan line 2487), claiming parity with `--live` mode. But under cassette mode the real `QdrantCorpus` exposes payloads via `get_post_mortem`; the collapse is a regression in `all_sources_in_allowed_domains` strictness, not a forced limitation. Either extend the read-side `Corpus` Protocol to expose payload sources for assertions, or document the strictness reduction in baseline v2 metadata. **[resolved]** Plan Task 6 now extends the `Corpus` Protocol with `async def lookup_sources(canonical_id) -> list[str]` (Step 6Pre/0a/0b/0c). `QdrantCorpus.lookup_sources` retrieves the `sources` field from the existing payload. The cassette runner (`_run_cassettes`) and live runner (`_run_live`) both pre-fetch a `sources_map: Mapping[candidate_id, list[str]]` after `run_query` and pass it to `_score_report`. `_allowed_hosts_for_candidate` is rewritten to consume the map; the previous "live-mode limitation" docstring note is removed because both modes now retain full assertion strictness.
- **P19 (tests)** — `test_round_trip_preserves_query_results` (plan lines 1366-1393) compares `sorted(read_text().splitlines())`. Float vector serialization through Qdrant HTTP/protobuf is not guaranteed byte-identical across versions or scroll orders. Compare deserialized objects with float tolerance instead.

### Cross-references to spec-level review

| Plan finding | Related spec finding | Relationship |
|---|---|---|
| P1 | B2 (cassette key omits tool list, schema, taxonomy) | B2's `[autofix]` was added to the spec but never wired into the runtime; plan implementation is incomplete |
| P4 | F20 (no cost ceiling on `eval-record`) | F20 resolved with `$2.00` default in spec; plan's pre-call check + post-call accumulation makes the cap soft + breaks its own unit test |
| P5 | F20 (cost ceiling) | Cost-ceiling guarantee silently doesn't apply to `eval-record-corpus`'s ingest stages |
| P6 | F22 (no Git LFS) | F22 `[discuss]` resolved via `.gitattributes`; plan needs a precommit gate to make the LFS guard load-bearing |
| P17 | G14 (layering violation: `recording.py` location) | G14 fixed by moving `recording.py` to `evals/`; plan reintroduces a similar inversion via lazy import in `fake.py`. **[resolved]** Plan now splits cassettes into `llm/cassettes.py` (key derivation) + `evals/cassettes.py` (format/IO); both directions are clean, lazy import deleted. |
