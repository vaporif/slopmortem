...
- **P12 (determinism)** — Schema version mismatch is hard-fail with no documented forward-compat policy. Adding a benign field later (e.g., `logprobs`) either bumps the version (invalidating every committed cassette → real $) or breaks readers. Document "minor schema bumps add fields, readers ignore unknowns" or check `version <= CASSETTE_SCHEMA_VERSION`.
...
| P17 | G14 (layering violation: `recording.py` location) | G14 fixed by moving `recording.py` to `evals/`; plan reintroduces a similar inversion via lazy import in `fake.py` |
