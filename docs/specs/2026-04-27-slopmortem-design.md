...
A Python CLI that takes a startup name and ~200-word description, finds similar startups that died within the last N years, and writes per-candidate post-mortems explaining why each is similar and where it diverges. The system is built around a deterministic pipeline of pure stage functions, with every LLM call routed through a single `LLMClient` abstraction and every embedding call routed through an `EmbeddingClient` abstraction. v1 uses the OpenAI Python SDK pointed at OpenRouter for LLM calls (chosen for model availability and one-key access to Sonnet/Haiku with a swap path to other providers), with native tool use, prompt caching passed through to the Anthropic backend, and bounded-concurrency `asyncio.gather` for ingest fan-out. OpenAI directly for embeddings. Qdrant runs as a local Docker service holding vectors and structured metadata; raw post-mortem text lives as markdown files on disk. Laminar instruments every stage and every LLM, embedding, tool, and corpus call so iteration on prompts and models has full visibility.
...
└────────────────────┬─────────────────────────────────────┬───────────────────────┘
       ...
       └────────┬────────────────┘      │            QUERY PIPELINE             │
                ...
                │     │             on non-"other" facets (qdrant-client≥1.17.1)│
...
─────────────────────────  CROSS-CUTTING LAYERS  ─────────────────────────────
    ...
    │  ┌──────────────────────────┐   ┌──────────────────────────┐  │
    │  │ OpenRouterClient (v1)    │   │ OpenAIEmbeddingClient v1 │  │
    │  │  openai SDK pointed at   │   │  text-embedding-3-small  │  │
    │  │  openrouter.ai/api/v1    │   │  via openai client       │  │
    │  │  chat.completions.create │   ├──────────────────────────┤  │
    │  │  native tool use:        │   │ FakeEmbeddingClient      │  │
    │  │   Pydantic-arg fns       │   │  (cassette, tests only)  │  │
    │  │   passed as tools=[...]  │   └──────────────────────────┘  │
    │  │  cache_control passed    │                                 │
    │  │   through to Anthropic   │   Both: retry+backoff, Laminar  │
    │  │   backends; cache hits   │   span, cost in pipeline_meta.  │
    │  │   read from OpenRouter   │                                 │
    │  │   usage extension        │                                 │
...
- Pros: every stage independently testable; tracing is uniform via decorators; the `LLMClient` Protocol keeps a backend swap (e.g. direct-Anthropic with Batches, or another OpenRouter-shaped provider) confined to one new class.
...
- Every stage is `async def`. SDK calls use the async clients: `AsyncOpenAI` (one instance pointed at OpenRouter for LLM, one at OpenAI for embeddings), `AsyncQdrantClient`. CPU-bound work (`fastembed` BM25 encoding) dispatches via `asyncio.to_thread` so the event loop stays responsive.
...
- Rationale: synthesis already fans out via `asyncio.gather` (see §Concurrency), and mixing sync stages forces either `asyncio.run` per call (loses the pool) or sync calls inside an async stage (blocks the loop and serializes the gather). Fully-async is the only shape that doesn't pessimize one of those two.

**Single LLMClient abstraction, OpenRouter via OpenAI SDK in v1**
- All LLM calls (facet extract, summarize, polish rerank, synthesis) go through `LLMClient.complete(prompt, *, tools=None, model=None, cache=None, response_format=None) -> CompletionResult` where `CompletionResult` carries `text: str`, `stop_reason: str`, `parsed: BaseModel | None`, and optional `cache_read_tokens`/`cache_creation_tokens` (best-effort; populated from `usage.prompt_tokens_details.cached_tokens` / `cache_write_tokens` when OpenRouter routes to an Anthropic backend). v1 implements this with `OpenRouterClient` over the `openai` Python SDK pointed at `https://openrouter.ai/api/v1`, calling `client.chat.completions.create(...)` with `tools=[...]` for native tool use. Tools are registered as plain Python callables; argument schemas are derived via `to_openai_input_schema(args_model)` in `slopmortem/llm/tools.py`, which calls `args_model.model_json_schema()`, inlines `$ref`/`$defs` via `jsonref.replace_refs(proxies=False, lazy_load=False)`, and strips draft-2020-12 metadata (`$schema`, `$defs`, `$id`). `Optional[T]` fields default to Pydantic's `anyOf:[T,null]` emission. OpenRouter explicitly transforms tool schemas before forwarding to non-OpenAI providers (Anthropic among them) and does not publish the transformation rules. The helper carries a config-driven fallback that can re-emit the field as `type:["T","null"]` if a startup probe shows the `anyOf` shape gets stripped or stringified by the routing layer. The probe is a one-shot tool call with an `Optional[str]` field (issued at LLMClient init when `OPENROUTER_PROBE_TOOL_SCHEMA=1`, default off in production), with `extra_body={"debug": {"echo_upstream_body": true}}` set so OpenRouter returns the body it forwarded; the helper compares the probe response against the original schema and logs which form was preserved. Fallback to the type-array form is not on by default. The assumption is that `anyOf` survives; the probe is the discipline that catches it if not. Anthropic-shape lock-in is real but contained: structured outputs and `cache_control` only behave as specced when OpenRouter routes to an Anthropic backend (the v1 default for Sonnet/Haiku). Routing to a non-Anthropic model is supported by the Protocol shape but the cache and grammar guarantees become best-effort, flagged at config-load time.
- Prompt caching is explicit: `cache_control={"type": "ephemeral"}` is included on the shared static system block (taxonomy, instructions, untrusted-document framing). OpenRouter passes `cache_control` through to Anthropic backends unchanged. Cache hits are **measured**, not assumed: cache-token counts are read off OpenRouter's `usage` extension on every response (read from `usage.prompt_tokens_details.cached_tokens` (cache hits, a subset of `prompt_tokens`) and `usage.prompt_tokens_details.cache_write_tokens` (cache creation); the LLMClient normalizes these into `CompletionResult.cache_read_tokens` / `cache_creation_tokens`. Total cost is read from `usage.cost` rather than reconstructed from rates, so cache discount accounting requires no client-side multiplier table.) and recorded on the Laminar span. The synthesis fan-out warms the cache deliberately by serializing the first call so the other four hit a populated cache rather than racing to write. The warm call asserts `cache_creation_tokens > 0` on its response. Anthropic's cache is eventually consistent across regions, and a 200 OK on the warm call doesn't guarantee the prefix is replicated to the pool serving the gather'd calls. If the assertion fails, one re-warm retry runs before the gather; if it fails twice, the pipeline emits `cache_warm_failed` and proceeds without the warm assumption (cost goes up by ~$0.04–0.08 for that one query, traceable in the span).
- Ingest fan-out is **bounded-concurrency `asyncio.gather`** under `anyio.CapacityLimiter(config.ingest_concurrency)` (default **20**, set in `slopmortem.toml`). 500 facet_extract + 500 summarize_for_rerank calls run as live HTTP calls. There is no Anthropic Batches API path because OpenRouter does not proxy it. The 1-hour TTL prompt cache (below) recovers most of the per-call cost the Batches discount would have captured; the residual gap is absorbed by `max_cost_usd_per_ingest = $15.00` (raised from $10 to cover the no-batch path with headroom).
- Prompt-cache interaction across the bounded fan-out: (a) the shared system block uses **1-hour TTL** `cache_control={"type":"ephemeral","ttl":"1h"}` so cache entries survive a long-running ingest (the cassette layer pins request bodies so any default change is loud rather than silent), (b) a single pre-fan-out warm call serializes ahead of the gather and asserts `cache_creation_tokens > 0`; the rest of the fan-out runs cache-hot, (c) the first 5 fan-out responses' `cache_read_tokens` / `cache_creation_tokens` are read off and logged. If `cache_read / (cache_read + cache_creation) < 0.80` on the warmed run, we know before spending the full ingest budget.
- Tool use loop: synthesis combines corpus tools with **structured outputs** in the same call. `tools=[get_post_mortem, search_corpus]` (+ optional Tavily) with `tool_choice="auto"`, plus `response_format={"type":"json_schema","json_schema":{"name":"Synthesis","schema": Synthesis.model_json_schema(),"strict": True}}` and `extra_body={"provider": {"require_parameters": True}}` (OpenAI-shape `response_format`; the `require_parameters` flag restricts routing to providers that report `response_format` support, so a fallback can't silently downgrade to prompted JSON. OpenRouter routes this to Anthropic's grammar-constrained sampling for Sonnet/Haiku). Per Anthropic's docs (the routed backend): "Grammars apply only to Claude's direct output, not to tool use calls. Grammar state resets between sections, allowing Claude to think freely while still producing structured output in the final response." When the model returns `finish_reason="tool_calls"`, the client executes the corpus tool's Python `fn` and re-injects the return value as a `tool` message (with `<untrusted_document>` framing). When `finish_reason="stop"`, the final assistant text is guaranteed schema-conformant JSON; `Synthesis.model_validate_json(...)` parses it. No `submit_synthesis` output tool, no penultimate-turn `tool_choice` flip. Loop bound (default 5 turns) still applies as runaway-protection; `finish_reason="length"`/`"content_filter"` are explicit failure modes with span events. Caveat: combined optional params across strict tools + output schema cap at 24 on the Anthropic backend, union-typed at 16. Corpus tools are NOT marked `strict:True` so only the synthesis schema's count matters.
- Auth: `OPENROUTER_API_KEY` from `.env` (gitignored), `SecretStr` in config; same surface as OpenAI and Tavily keys. OpenRouter forwards to the underlying provider; no per-provider key is needed for the routed Anthropic models in v1.
- Pros: one API key covers Sonnet/Haiku plus a swap path to other providers without code change (model availability when Anthropic is rate-limited or a model is region-gated); no subprocess cold-start tax; cache visibility via OpenRouter's `usage` extension; cassettes record OpenAI-shape responses; tool surface is just Python, with no MCP transport, no `--allowedTools` allowlist drift, no `claude -p` version pinning.
- Cons: routing layer adds one network hop with no per-token markup over direct-Anthropic pricing (`prices.yml` carries Anthropic's posted rates verbatim); a 5.5% platform fee applies on PAYG credit top-ups and is modelled separately as a deposit-time multiplier on the per-query and per-ingest cost ceilings, not folded into the per-token table; no Message Batches API, so ingest cost is ~30–50% higher than direct-Anthropic-batched, recovered partly by the 1h TTL prompt cache; cache-token field shape depends on OpenRouter's usage extension and may evolve across their API revisions (cassette body-match catches this loudly).
- **`ClaudeCliClient` deferred**: a subprocess-based implementation was an earlier candidate but lost on three counts: (a) 1–3s cold start × hundreds of ingest calls, (b) prompt-cache hits are not exposed in `claude -p` output JSON so cache effectiveness is unmeasurable, (c) the synthesis MCP boundary recreated tool-call infrastructure that the SDK provides natively. It remains a possible follow-on for users who prefer Claude Code session auth over an API key, but it is out of v1 scope and not in Task #2.

**Hybrid retrieval (BM25 + dense, RRF fused), no HyDE**
- Qdrant native hybrid via dense + sparse vectors fused server-side with Reciprocal Rank Fusion, wrapped in a top-level `FormulaQuery` for facet boost (`Prefetch` + `FusionQuery(fusion=RRF)` nested under `FormulaQuery`, requires `qdrant-client>=1.17.1` and server `qdrant>=1.17.1`). Dense embeddings come directly from the user's description, no HyDE expansion. The forward-pitch ↔ past-obit modality gap is real but the rerank stage at `K_retrieve=30 → N_synthesize=5` absorbs retrieval slack. HyDE's typical lift is at recall@10, not recall@30, and it costs a per-query Haiku call plus a documented trope-injection failure mode (Haiku rewrites the pitch as a "post-mortem opening" stuffed with "ran out of runway", "scaled too fast", biasing retrieval toward generic-failure clusters). BM25 covers surface-vocabulary mismatch independently. May revisit in v2 if real-pitch recall turns out poor in measurement.
- Dense embeddings: OpenAI `text-embedding-3-small` (1536 dims) routed through `EmbeddingClient` (Protocol with the same retry/backoff/Laminar/cost-tracking surface as `LLMClient`). Sparse embeddings: `fastembed` BM25 model. Qdrant collection MUST be created with `modifier=models.Modifier.IDF` on the sparse vector config (fastembed emits term-frequencies without IDF; Qdrant computes IDF at query time). The embedding provider is configured in `config.py` so a local sentence-transformers model can swap in later. Vector dimension is **not** independently configured: an `EMBED_DIMS` map in `slopmortem/llm/openai_embeddings.py` (`{"text-embedding-3-small": 1536, "text-embedding-3-large": 3072, ...}`) is the single source of truth, read by both `EmbeddingClient` (asserts on returned vector length) and `ensure_collection` (passes `dim=EMBED_DIMS[settings.embed_model_id]`). `ensure_collection` raises `ValueError("dim mismatch …")` if the existing collection's dim disagrees with the configured model. This prevents a silent corruption window when `embed_model_id` is changed without dropping `data/qdrant/`. Adding a new embedding model = one entry in `EMBED_DIMS`.
- Soft-boost facet matching uses Qdrant's **`FormulaQuery` / Score Boosting** (1.14+) wrapping a nested RRF prefetch. The outer `query=FormulaQuery(...)` adds a per-candidate boost when the candidate's payload matches non-`"other"` facets; `$score` inside the formula resolves to the RRF-fused dense+sparse score from the inner prefetch. One round-trip, no Python merge. An earlier draft used a *third filtered Prefetch* fused via RRF as a rank-lift, which works (filtered candidates appear in the fused list and average up) but bends the abstraction. Qdrant 1.14 ships `FormulaQuery` as the documented primitive for exactly this intent. `Filter.should` is *filtering* (logical OR over conditions), **not** score boosting; adding facets to `should` would silently no-op (when `must` is present) or hard-filter (when it isn't), neither of which is the desired soft boost. `"other"` is skipped from the boost because boosting on it matches every other-bucketed entry indiscriminately and is actively harmful, not neutral. Requires `qdrant-client>=1.17.1` and server `qdrant>=1.17.1`.
...
- Pros: catches proper nouns and rare terms (specific tech, niche markets) that pure embedding misses; deterministic at the query side (no LLM step before retrieval), so the same input always produces the same query vectors and the same retrieval result.
- Cons: more moving parts than pure dense; OpenAI embeddings need an API key (one of the few external deps); for unusually terse or jargon-light pitches retrieval recall may be poor. v2 follow-up if measurement confirms.
...
- Output is enforced via OpenRouter's OpenAI-shape **structured outputs** primitive (which routes to Anthropic's GA grammar-constrained sampling on Sonnet/Haiku): every structured-output call passes `response_format={"type":"json_schema","json_schema":{"name": Model.__name__, "schema": to_strict_response_schema(Model), "strict": True}}` to `chat.completions.create` and parses the assistant text via `Model.model_validate_json(...)`. `to_strict_response_schema` (in `slopmortem/llm/tools.py`) is required because Pydantic v2 omits any field with a default (including `Optional[T] = None`) from `required`, but OpenAI strict mode mandates that every property be in `required` (nullability is expressed via `anyOf:[T,null]`, not by absence). The helper inlines `$ref`/`$defs`, strips draft metadata, and force-adds every property to `required` while preserving `anyOf:[T,null]` verbatim. It is idempotent for models without Optional-default fields (`Synthesis`, `LlmRerankResult`); the load-bearing case is `Facets` (5 Optional fields with `None` defaults). SDK helper wrappers (`client.beta.chat.completions.parse(...)`) are **not** used in v1. Picking one form keeps the cassette surface uniform; mixing both produced subtly different request bodies and broke vcrpy body-match on replay. For `llm_rerank`: `schema = LlmRerankResult.model_json_schema()`, no `tools=[...]`, no termination dance. An earlier draft used a forced-`tool_choice` output tool (`submit_llm_rerank`); that pattern still works but is now legacy. No JSON-in-prose parsing.
...
- Default sources (`slopmortem ingest` with no flags): a bundled curated YAML list of hand-vetted post-mortem URLs (parsed via `trafilatura` for content extraction with a length floor + domain blocklist), plus the Hacker News Algolia API for ongoing obituary coverage. Both are real APIs / static inputs, no fragile per-site scraping. **HN endpoint is pinned to `https://hn.algolia.com/api/v1/search_by_date`** (chronological, newest-first) rather than `/search` (relevance-ranked). Ongoing obituary coverage is recency-driven, and relevance ranking would re-surface the same long-tail popular threads on every ingest. Query params: `tags=story`, `query=<startup-name OR "post-mortem" OR "shut down" …>`, `numericFilters=created_at_i>=<since-epoch>` for incremental ingests, paginated via `page=` until `nbPages` exhausted.
...
slopmortem/
  ...
  stages/
    ...
    llm_rerank.py          # one Sonnet call, K_retrieve → N_synthesize directly,
                           #   response_format=json_schema(LlmRerankResult),
  ...
  llm/
    ...
    tools.py               # synthesis_tools(config: Config) -> list[ToolSpec] factory
                           ...
                           #     §Architecture).
                           #   Used by ToolSpec → OpenAI-shape tool-schema conversion (the
                           #   underlying Anthropic backend accepts this shape via OpenRouter).
  ...
  # the OpenAI SDK (pointed at OpenRouter) directly (see slopmortem/llm/tools.py).
  # There is no MCP server in the v1 query path. An optional MCP wrapper exposing the
  # same get_post_mortem / search_corpus functions to external Claude Code sessions
  # is tracked under Open questions but not part of v1.
  ...
  budget.py                # per-invocation cost cap; reserve/settle under asyncio.Lock so the
                           # cap holds under gather; raises BudgetExceeded from LLMClient/EmbeddingClient
  config.py                # pydantic-settings v2: `Config(BaseSettings)` with ordered
                           #   sources via SettingsConfigDict — committed defaults
                           #   `slopmortem.toml` (TomlConfigSettingsSource), gitignored
                           #   user override `slopmortem.local.toml` (TomlConfigSettingsSource,
                           #   higher precedence; absent is fine), then `.env` for secrets
                           #   only (OPENROUTER_API_KEY, OPENAI_API_KEY, TAVILY_API_KEY,
                           #   LAMINAR_API_KEY), then process env vars (highest precedence,
                           #   for CI overrides). `extra="forbid"` so typos in TOML fail loud.
                           #   Tunables (all live in TOML — never hardcoded in Python):
                           #     K_retrieve (30), N_synthesize (5),
                           #     ingest_concurrency (20), facet_boost (0.01),
                           #     rrf_k (60),
                           #     model_facet, model_summarize, model_rerank, model_synthesize (all values are OpenRouter slugs, e.g. `anthropic/claude-sonnet-4.6`, `anthropic/claude-haiku-4.5`),
                           ...
                           #   `slopmortem.toml` in the repo root carries the defaults above
                           #   so they are reviewable in PRs; user-local tunings go to
                           #   `slopmortem.local.toml` (gitignored).
  evals/
...
qdrant.query_points(
  ...
  # qdrant-client>=1.17.1 and server qdrant>=1.17.1.
  prefetch=Prefetch(
    prefetch=[
...
llm_rerank(candidates.summary, description, query_facets, llm=sonnet)
  → 1 SDK chat.completions.create call (OpenAI SDK pointed at OpenRouter)
    covering all K_retrieve candidates with
    response_format={"type":"json_schema",
                     "json_schema":{"name":"LlmRerankResult",
                                    "schema": LlmRerankResult.model_json_schema(),
                                    "strict": True}},
    extra_body={"provider": {"require_parameters": True}}.
    No tools, no corpus access — pure structured output. OpenRouter routes
    this to the Anthropic backend's grammar-constrained sampling, which
    guarantees schema-conformant JSON; the assistant text is parsed
    via LlmRerankResult.model_validate_json(content). (The SDK's
    helper wrappers (`client.beta.chat.completions.parse(...)` /
...
synthesize_all(top_n, query_ctx, llm=sonnet, tools=synthesis_tools(config))
  ...
  → tools = [get_post_mortem, search_corpus]
    ...
    finish_reason=="stop", final text is schema-conformant JSON and
    ...
    finish_reason in {"length","content_filter"} → hard failure with span event.
...
- Synthesize: cache-warm pattern. The first `chat.completions.create` call runs alone to populate the prompt cache on the routed Anthropic backend. `CompletionResult.cache_creation_tokens` is asserted `> 0` on the warm-call response; if zero (Anthropic cache eventual consistency, OpenRouter routing skew, or missing usage extension), one re-warm retry runs before the gather. The remaining `N_synthesize - 1` run via `asyncio.gather` wrapped in **`anyio.CapacityLimiter(N_synthesize)` by default**. N=5 syntheses × up to 5 tool turns × (corpus + ≤2 Tavily) is ~35 outbound requests in flight, well above any reasonable per-key concurrency floor under retry; the limiter is shipped on by default rather than "revisit if storms observed." All calls are async HTTP via the SDK; no subprocess management. Ctrl-C cancels the asyncio task group; in-flight HTTP requests abort via the SDK's cancel surface.
- LLM rerank is one call, no fan-out.
- Ingest LLM calls (~1000 facet+summarize per re-seed) run as bounded-concurrency `asyncio.gather` under `anyio.CapacityLimiter(config.ingest_concurrency)` (default 20). There is no Anthropic Batches API path because OpenRouter does not proxy it; the live-call fan-out is the only path. Each call is independent, retries+budget apply per-call, and Ctrl-C cancels in-flight tasks via the asyncio task group with no orphaned charges (OpenRouter bills per completed request; a cancelled in-flight HTTP request that hasn't reached the backend is not billed; one already at the backend may complete and bill, an acceptable tail).

### Failure handling

- `LLMClient` retries with exponential backoff on transient failures (HTTP 5xx, network timeout). Tool-call args validate against the tool's `parameters` schema server-side at the routed backend, so structured-output schema drift never reaches our Pydantic boundary as a runtime exception. `args_model.model_validate(tool_call.arguments)` after the API call is defense-in-depth and is not expected to fail in practice; if it does (signaling a schema mismatch between Pydantic and the schema we sent), it raises immediately without retry because retrying won't change the schema we send. Corpus-tool results that the model returns malformed (e.g. invalid args to `search_corpus`) are reported back to the model as a `tool` message with an error string and consume one tool-loop turn, NOT a retry. Max 3 retries per call across the transient-failure classes. Auth-class failures (`401`/`403` from the SDK, expired or missing API key) are detected separately and short-circuit retries with a user-facing error rather than burning the budget.
- Rate-limit detection: SDK `RateLimitError` (HTTP 429) is handled by the openai SDK's built-in retry path. The SDK reads `Retry-After` / `retry-after-ms` if the response carries them and falls back to exponential backoff with jitter otherwise; OpenRouter does not document whether it forwards the upstream provider's `Retry-After` header, so the fallback path is treated as the steady-state expectation, not a failure mode. Anthropic upstream overloads (`overloaded_error`, native HTTP 529) surface on the OpenRouter path as one of two shapes: (a) HTTP **502** pre-stream (mapped to a transient retry alongside other 5xx), or (b) a mid-stream SSE chunk at HTTP 200 with `finish_reason: "error"` and `error.code` carrying the upstream code in `metadata.raw`. The LLMClient inspects every streamed final chunk for the error finish-reason and re-raises a synthetic transient exception so the same retry path applies. HTTP **503** ("no available provider meets routing requirements") is treated as fatal; retrying does not change the routing pool. HTTP **402** (insufficient credits) is fatal and short-circuits the budget loop. After max retries the candidate drops per the rule below.
- Per-invocation budget: every `LLMClient.complete` and `EmbeddingClient.embed` call accumulates `cost_usd` against a `Budget` object initialized from `config.max_cost_usd_per_query` (default $2.00) or `_per_ingest` (default $15.00). Cost is read from `usage.cost` (OpenRouter-reported, already net of cache savings); the per-model price table in `prices.yml` is used only for the pre-call reservation upper bound and as a sanity-check against `usage.cost` (an alarm fires if they diverge by more than 5%, signalling either a price-table drift or an OpenRouter pricing change). `usage.prompt_tokens`, `usage.completion_tokens`, and the cache-token counts are still recorded on the Laminar span for cache-hit ratio reporting. To make the cap hold under `asyncio.gather` (synthesize and ingest fan-out), each API roundtrip uses a reserve→settle cycle: before the call, under `Budget.lock` (`asyncio.Lock`), compute a pessimistic upper bound `reserve = prompt_tokens × input_price + max_completion_tokens × output_price` (no cache discount applied; `prompt_tokens` known from the outgoing message payload, `max_completion_tokens` from the call param), check `remaining >= reserve`, debit, return a reservation id. On response, settle to actual measured cost (refunds the cache discount and any unused output tokens; tops up if usage exceeded the bound, which only happens on price-table drift). Reservation is per HTTP roundtrip, so each tool-loop turn reserves and settles independently. Insufficient remaining at reserve time raises `BudgetExceeded` immediately, the partial report renders with what's complete, and the trace records `budget_exceeded=True`.
...
`Synthesis` is the `response_format.json_schema` for the synthesize
call. OpenRouter routes structured outputs to Anthropic's grammar-constrained
sampling on Sonnet/Haiku, which guarantees the final assistant text is
schema-conformant JSON; the LLMClient parses it. There is no JSON-in-prose
parsing path and no output-tool indirection.
...
Laminar wraps the entire system. Initialization is a no-op when env vars are unset, so the pipeline runs identically without it. `tracing.py` parses `LMNR_BASE_URL` with `urllib.parse`, resolves the host via `socket.getaddrinfo(host, None)` (IPv4 + IPv6; `gethostbyname` is IPv4-only and would let an AAAA-only `::1` slip past while an A record sits alongside), and requires **all** resolved addresses to satisfy `ipaddress.ip_address(...).is_loopback` (covers `127.0.0.0/8`, `::1`, and IPv4-mapped variants) OR be an exact match for a configured private host. **String-prefix checks are not used**; `http://localhost.attacker.com` would defeat them via DNS rebinding. **IP-pinning into the URL passed to `Laminar.init` is deferred to v2** (see `docs/specs/2026-04-28-design-review-issues.md` #6 for the mechanism and rationale): the Laminar SDK manages its own httpx client + OTel exporter and does not accept a custom resolver/transport, so the `slopmortem/http.py:safe_get` resolver cannot be bound into it without upstream changes. v1 therefore does the resolve-and-validate check at init only; a TOCTOU rebind window remains between that check and the SDK's first connect. On the loopback default (no env vars / `LMNR_BASE_URL=http://localhost:…`) the runtime exposure is small. On the `LMNR_ALLOW_REMOTE=1` path the residual window is real and accepted for v1; remote URLs emit a startup banner and the deployment is responsible for trusting its own DNS path. Tracking: v2 hardening adds the IP-pin once an upstream hook exists or the OTel exporter is replaced.
...
| 500 × facet_extract (Haiku, 1.5K input + 1.5K output, cache-warmed) | ~$3.50 |
| 500 × summarize_for_rerank (Haiku, 2K input + 0.4K output, cache-warmed) | ~$1.60 |
...
| **Total initial (live ingest fan-out)** | **~$7.50** |
| **Steady-state (HN feed, ~10 entries/week)** | **~$0.10/week** |

OpenRouter routes Haiku/Sonnet calls to Anthropic at pass-through pricing (no per-token markup; `prices.yml` carries Anthropic's posted rates verbatim, with a separate 5.5% PAYG-credit-deposit fee modelled at the budget-ceiling layer). The Anthropic Message Batches API is **not** available through OpenRouter, so the 50% batch discount the earlier draft assumed is gone. The figures above are for the live bounded-concurrency fan-out (`ingest_concurrency = 20`). The 1-hour-TTL prompt cache on the shared system block recovers the bulk of what Batches would have saved (cache_read tokens are ~10% of base input cost), so the net delta vs the old batched estimate is ~+50%, not 2×. The embedding row was previously over-estimated at $0.35; corrected against text-embedding-3-small pricing ($0.02/1M tokens) on ~1.9M tokens for 500 × 5 chunks.

Per-invocation budget: `config.max_cost_usd_per_ingest` defaults to **$15.00** (covers live-fan-out initial seeding + 100% headroom for re-merges and retries; raised from $10 because the no-Batches path has less cushion).
...
Earlier drafts assumed `claude -p` subprocess cold-starts of 1–3s per call. Going through the OpenAI SDK (pointed at OpenRouter) eliminates that tax. There is no subprocess to spawn, and HTTP keep-alive is reused across calls in the same process. OpenRouter adds one network hop on top of direct-Anthropic; in practice this is sub-200ms and lost in the noise of Sonnet's 8–15s synthesis latency.
...
The system ingests third-party scraped content and feeds it into LLMs that have tool access. The corpus is the threat surface; the synthesis call (LLM with tool use enabled) is the privileged sink. Every defense below is required.

**Prompt injection (corpus → synthesis)**

v1 ships the OWASP / Anthropic / AWS baseline. The CLI has no private data and no write actions, so the trifecta is small. Architectural upgrades (spotlighting/datamarking, dual-LLM IFC, large adversarial test corpus) are tracked under §Open questions for v2.
...
**Secrets**
- All API keys (OpenRouter, OpenAI for embeddings, Tavily, Laminar) are `pydantic.SecretStr` fields in `config.py`, sourced from env vars or `.env` (gitignored) only, never the TOML config. `Config.__repr__` redacts. The Laminar `@observe` instrumentation is configured to never capture `Config` objects in spans.

**Cassettes**
- Cassette write filter scrubs known secret formats with hyphen-aware patterns: `(?i)sk-(?:ant-(?:admin\d+-|api\d+-)?|proj-|svcacct-|or-v1-)?[A-Za-z0-9_\-]{20,}` (Anthropic incl. admin keys + OpenAI legacy/proj/svcacct + OpenRouter `sk-or-v1-…`), `tvly-[A-Za-z0-9]{20,}` (Tavily), `lmnr_[A-Za-z0-9]{20,}` (Laminar), `(?:rk_live|sk_live|sk_test|rk_test)_[A-Za-z0-9]{24,}` (Stripe), `eyJ[A-Za-z0-9_-]{10,}\.eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}` (JWTs: header.payload.signature), `AKIA[0-9A-Z]{16}` / `ASIA[0-9A-Z]{16}` (AWS), `ya29\.[A-Za-z0-9_\-]+` (GCP), `ghp_[A-Za-z0-9]{36}` (GitHub), `bearer\s+\S+`, `api[_-]?key["\s:=]+\S+`. Header-name allowlist scrub: any value of `Authorization`, `x-api-key`, `x-anthropic-api-key`, `openai-api-key`, `openrouter-api-key` is redacted regardless of value pattern. Env var values and home-directory paths also scrubbed. `RECORD=1` requires `REVIEW=1` on the same invocation. Pre-commit hook scans `tests/fixtures/cassettes/` for residual secret patterns.

**Laminar URL guard**
- `tracing.py` refuses to initialize unless `LMNR_BASE_URL`'s host resolves (via `socket.getaddrinfo(host, None)` for IPv4 + IPv6, where `gethostbyname` is IPv4-only and would let `::1` slip past) to addresses where **all** entries are loopback (`ipaddress.is_loopback` covers `127.0.0.0/8`, `::1`, IPv4-mapped) or match a configured private-host allowlist exactly. **Not** a `startswith` string check; `http://localhost.attacker.com` defeats prefix matching. **IP-pinning into the URL passed to `Laminar.init` is deferred to v2**. The Laminar SDK does not expose an `http_client` / `transport` parameter (verified against `lmnr-ai/lmnr-python`), so the `safe_get` resolver cannot be bound into the SDK's connection pool in v1. A TOCTOU rebind window therefore remains between resolve-and-validate at init and the SDK's first connect; on the loopback default the exposure is small, on `LMNR_ALLOW_REMOTE=1` it is accepted as a known v1 limitation. Override the loopback guard with `LMNR_ALLOW_REMOTE=1`, which logs `tracing → <host>` to stderr at startup. v2 will add the IP-pin once an upstream hook exists. See `docs/specs/2026-04-28-design-review-issues.md` #6.

**Outbound HTTP / SSRF**
- All outbound HTTP (Tavily fetches, Wayback fetches, source scrapes, the Laminar URL guard) goes through a single `slopmortem/http.py:safe_get(...)` wrapper. The wrapper resolves DNS via `socket.getaddrinfo(host, None)` (IPv4 + IPv6) and refuses any resolved address that is loopback / link-local / RFC1918 / `169.254.0.0/16` (incl. AWS/GCP/Azure IMDS) / IPv6 ULA / `metadata.google.internal` / `metadata.azure.com` / `100.64.0.0/10` (CGNAT). It refuses any non-`http(s)` scheme (`file://`, `gopher://`, `dict://`, etc). The resolved IP is pinned into the request via a custom httpx resolver to prevent TOCTOU rebinds between resolution and connect. Without this guard, Tavily-returned URLs could trivially target `http://169.254.169.254/latest/meta-data/` (AWS IMDS) or `http://localhost:6333/` (the local Qdrant). robots.txt and UA aren't security. `robots.txt` etiquette and per-host token bucket (≤1 rps default) layer on top, but are not security controls. UA identifies as `slopmortem/<version> (+<repo url>)`. `If-Modified-Since` / `ETag` honored to avoid re-fetching unchanged URLs.

**Resource limits (OWASP LLM10)**
- Per-doc inline cap before synthesis: each candidate's `combined_text` is token-counted via the active model's tokenizer and **hard-truncated** at `config.max_doc_tokens` (default 50000) before the `<untrusted_document>` wrapping. Truncation emits a `corpus.doc_truncated` span event with `canonical_id` + observed/cap token counts. Bounds the token-bomb DoS where a hostile 50K+ token corpus doc explodes synthesis input cost; per-doc rather than per-call because synthesis inlines one canonical per call. Per-call budget enforcement (already in place via `Budget.lock` reserve/settle) layers on top.

**System prompt content (OWASP LLM07)**
- No system prompt in this codebase contains secrets, credentials, PII, or private data. The synthesis system prompt is the `<untrusted_document>`-handling instruction; the rerank/facet system prompts are taxonomy-bound rubrics. Leakage is therefore not a confidentiality threat by construction; review-enforced (any future addition of sensitive content to a system prompt is rejected at PR review).

**Embedding weaknesses (OWASP LLM08, partial)**
- Three v1 mitigations cover the bulk-slop and content-quality vectors: ingest-time `slop_classify` (Binoculars; quarantines high-score docs), `provenance="curated_real"` payload tag for the curated YAML class, and the 500-char length floor + platform-domain blocklist that drops thin/farmed pages before embedding. Full adversarial-embedding coverage (similarity attacks, embedding inversion, retrieval-poisoning corpora) needs adversarial test fixtures and is tracked in v2 hardening (§v2 → Slop / corpus-hygiene upgrades).

**Curated YAML provenance**
- `slopmortem/corpus/sources/curated/post_mortems.yml` requires `CODEOWNERS` review. Each row carries `submitted_by`, `reviewed_by`, `content_sha256_at_review`. Ingest re-fetches and compares against the reviewed hash; on mismatch the entry is routed to `quarantine_journal` with `quarantine_reason="curated_drift"` (NOT embedded into Qdrant), a `corpus.poisoning_warning` span event is emitted, and ingest exits non-zero after writing a stderr summary listing each drifted entry's URL and observed/expected sha256. `--accept-corpus-drift` overrides the gate and ingests drifted entries anyway (the journal row is still written for audit). Per-entry resolution: re-review the upstream URL, then drop it from the YAML or update its `content_sha256_at_review` and rerun. Granularity is per-entry (one drift does not block the rest of ingest from completing through the journal-write step before the non-zero exit).

## Testing strategy

- **Unit tests per stage**: each takes a `FakeLLMClient` (cassette-backed) and a `FakeCorpus` (or a tiny fixture-loaded real one). Cassettes recorded via `RECORD=1 REVIEW=1 pytest`, committed, replayed on CI. Cassette miss = loud failure with recording hint, never a silent live call. A **cassette-miss meta-test** asserts that requesting a non-existent cassette under `RUN_LIVE` unset raises with the recording hint, guarding against accidental fallthrough to a real API call when a cassette filename drifts.
...
- **Drift control**: a `make smoke-live` target runs the `RUN_LIVE=1` E2E test against OpenRouter (live network) weekly (manual trigger acceptable; not on CI). Cassette regeneration after drift is a deliberate batch operation, not per-test.
- **Stage-specific assertions:**
  ...
  - `llm_rerank`: cassette-based, all K_retrieve candidates passed in (not a truncated subset), top-N_synthesize selection respects rubric ordering, per-perspective scores populated, output emerges as schema-conformant JSON via `response_format.json_schema` (not parsed JSON-in-prose). Summary field used (not full body).
...
- **Atomicity tests**: kill-switch test injects failure between markdown write and qdrant upsert, assert next ingest run completes the merge. Reconcile test: corrupt one of (markdown, qdrant), assert `slopmortem ingest --reconcile` reports and repairs.
...
- **Direct-Anthropic + Batches optimization**: `OpenRouterClient` is v1 default; a `DirectAnthropicClient` against `anthropic` SDK with the Message Batches API would recover the 50% ingest discount and pure pass-through pricing. Worth doing only if ingest cost becomes a real-world pain point (steady-state cost is ~$0.10/week, so likely never).
...
- **Single-call synthesis optimization**: synthesis currently runs N SDK calls (one warm + N-1 parallel). A single call covering all N candidates with structured output could save wall-clock at the cost of losing per-candidate retry/parallelism. Decision deferred until first real latency measurements.
...
- **HyDE (v2 candidate)**: v1 ships without HyDE. Rerank at K=30 → N=5 should absorb retrieval slack and the trope-injection failure mode (Haiku rewriting pitches into generic "ran out of runway" post-mortems) is concrete while the modality-gap concern is hypothetical. If real-pitch recall turns out poor in v2 measurement, add back behind a flag and measure the delta vs baseline before flipping default.
- **Confirm ingest cache hit rate**: first ingest run logs the warmed fan-out's per-response `usage.prompt_tokens_details.cached_tokens` / `cache_write_tokens` (the LLMClient normalizes these into `CompletionResult.cache_read_tokens` / `cache_creation_tokens`). Target ≥80% read ratio on the shared system block. If it underperforms, revisit: longer warm sequence, lower `ingest_concurrency`, or accept the higher cost and raise `max_cost_usd_per_ingest`.
...
- **Skip_key two-tier split for rank-version bumps**: today `skip_key` includes `reliability_rank_version`, so bumping it forces full re-derivation of every canonical on the next reconcile even when the resulting `combined_text` is byte-identical (single-source canonicals; rank-equivalent edits). Fix: split into `merge_skip_key=(sha256(combined_text), reliability_rank_version)` and `derivation_skip_key=(sha256(combined_text), facet_prompt_hash, summarize_prompt_hash, haiku_model_id, embed_model_id, chunk_strategy_version, taxonomy_version)`; on rank bump, always recompute combined_text but skip LLM/embedding work when the derivation key matches. Deferred. Rank-version bumps are rare and steady-state cost is ~$0.10/week; revisit if reconcile gets slow or rank bumps become frequent.
...
- **Adversarial embedding tests (OWASP LLM08)**: similarity-attack corpora, embedding-inversion probes, retrieval-poisoning fixtures. v1 ships only the bulk-slop classifier + provenance + length floor (partial coverage); full coverage requires curated adversarial datasets that don't yet exist for the post-mortem domain.
...
All tasks are Python. Per the agent type selection guide, Python isn't a listed specialty, so `general-purpose` is the right default.
...
| 1 | **Foundation**: pydantic-settings v2 with TOML sources (`slopmortem.toml` committed defaults + gitignored `slopmortem.local.toml` override + `.env` for secrets, `extra="forbid"`), all shared models, `LLMClient` + `EmbeddingClient` + `Corpus` + `ToolSpec` Protocols, **`CompletionResult` dataclass** (`text`, `stop_reason`, `parsed`, optional `cache_read_tokens`/`cache_creation_tokens`), **synthesis tool signatures** (`get_post_mortem`, `search_corpus` Pydantic arg models + return shapes), **`to_openai_input_schema(args_model)` helper in `slopmortem/llm/tools.py`** (jsonref-based `$ref` inlining + `$schema`/`$defs`/`$id` strip; round-trip test: Pydantic → schema → fake tool-call → parse → Pydantic, identical shape; regression test that `Optional[T]` keeps `anyOf:[T,null]`), **`to_strict_response_schema(model)` helper in same module** (force-adds every property to `required` for OpenAI strict-mode `response_format` — Pydantic v2 omits `Optional[T] = None` fields from `required`, but strict mode requires every property; nullability stays expressed via `anyOf:[T,null]`; idempotent for models without Optional defaults), `MergeState`, `safe_path`, `Budget`, `tracing.py` (with LMNR_BASE_URL guard). Adds `jsonref` to dependencies. | **G1** | general-purpose | Python |
...
| 7 | Stages: `retrieve` (NULL-aware date filter, FormulaQuery facet boost over RRF-fused dense+sparse, real-only-floor prefetch), `llm_rerank` (single Sonnet call K_retrieve→N_synthesize via `response_format=json_schema(LlmRerankResult)`, no tools, multi-perspective scoring) | post-G2 | general-purpose | Python |
| 8 | Stages: `synthesize` (inlined body, `<untrusted_document>` wrapping of body and tool results, in-process corpus tools registered with SDK, `response_format=json_schema(Synthesis)` coexisting with tools, cache-warm pattern, sources host-allowlist filter, Tavily ≤2 calls/synthesis budget), `render` (structural-only snapshots, autolink/image stripping) | post-G2 | general-purpose | Python |
