...
- All LLM calls (facet extract, summarize, polish rerank, synthesis) go through `LLMClient.complete(prompt, *, tools=None, model=None, cache=None, response_format=None) -> CompletionResult` where `CompletionResult` carries `text: str`, `stop_reason: str`, `parsed: BaseModel | None`, and optional `cache_read_tokens`/`cache_creation_tokens` (best-effort; populated when the routed backend reports them). v1 implements this with `OpenRouterClient` over the `openai` Python SDK pointed at `https://openrouter.ai/api/v1`, calling `client.chat.completions.create(...)` with `tools=[...]` for native tool use. Tools are registered as plain Python callables; argument schemas are derived via `to_openai_input_schema(args_model)` in `slopmortem/llm/tools.py`, which calls `args_model.model_json_schema()`, inlines `$ref`/`$defs` via `jsonref.replace_refs(proxies=False, lazy_load=False)`, and strips draft-2020-12 metadata (`$schema`, `$defs`, `$id`). `Optional[T]` fields preserve Pydantic's `anyOf:[T,null]` shape verbatim — rewriting to `type:[T,null]` has been observed to *increase* invalid-JSON output rates, so the helper deliberately leaves it alone. Anthropic-shape lock-in is real but contained: structured outputs and `cache_control` only behave as specced when OpenRouter routes to an Anthropic backend (the v1 default for Sonnet/Haiku). Routing to a non-Anthropic model is supported by the Protocol shape but the cache and grammar guarantees become best-effort — flagged at config-load time.
- Prompt caching is explicit: `cache_control={"type": "ephemeral"}` is included on the shared static system block (taxonomy, instructions, untrusted-document framing). OpenRouter passes `cache_control` through to Anthropic backends unchanged. Cache hits are **measured**, not assumed: cache-token counts are read off OpenRouter's `usage` extension on every response (OpenRouter exposes the underlying provider's cache fields under its own usage shape — the LLMClient extracts and normalizes them into `CompletionResult.cache_read_tokens` / `cache_creation_tokens`) and recorded on the Laminar span. The synthesis fan-out warms the cache deliberately by serializing the first call so the other four hit a populated cache rather than racing to write. The warm call asserts `cache_creation_tokens > 0` on its response — Anthropic's cache is eventually consistent across regions, and a 200 OK on the warm call doesn't guarantee the prefix is replicated to the pool serving the gather'd calls. If the assertion fails, one re-warm retry runs before the gather; if it fails twice, the pipeline emits `cache_warm_failed` and proceeds without the warm assumption (cost goes up by ~$0.04–0.08 for that one query, traceable in the span).
...
slopmortem/
  ...
  config.py                # pydantic-settings v2: `Config(BaseSettings)` with ordered
                           ...
                           #     rrf_k (60),
                           #     model_facet, model_summarize, model_rerank, model_synthesize,
...
- **Confirm ingest cache hit rate** — first ingest run logs the warmed fan-out's per-response `cache_read_tokens` / `cache_creation_tokens` (extracted from OpenRouter's usage extension). Target ≥80% read ratio on the shared system block. If it underperforms, revisit: longer warm sequence, lower `ingest_concurrency`, or accept the higher cost and raise `max_cost_usd_per_ingest`.
