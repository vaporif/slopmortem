...
def combined_text(sections: list[Section]) -> str:
    """Return merged text sorted by ``(reliability_rank, source_id)``, with ``## <source>:<source_id>`` provenance heads."""
    if not sections:
        return ""
...
def combined_hash(sections: list[Section]) -> str:
    """First 16 hex chars of sha256 over :func:`combined_text`; drives the skip-key ``content_hash``."""
