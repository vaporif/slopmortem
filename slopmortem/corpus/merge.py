...
def _row_to_dict(row: sqlite3.Row) -> dict[str, Any]:  # type: ignore[explicit-any]
...
class MergeJournal:
    ...
    def _fetch_aliases_sync(self, canonical_id: str) -> list[dict[str, Any]]:  # type: ignore[explicit-any]
        with _connect(self._db) as conn:
            cur = conn.execute(
                """
    ...
    def _write_quarantine_sync(
        ...
        with _connect(self._db) as conn:
            conn.execute(
                ...
                """,
                (content_sha256, source, source_id, reason, slop_score, _utcnow_iso()),
            )
